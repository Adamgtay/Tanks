...
def display_unit(unit, colour, x, y, char_spacing_x,char_spacing_y):
    ...
    for line in unit:
        ...
        for char in line:
            ...
            start_x += max_char_width + char_spacing_x  # Use the width of the widest character for spacing
            #char_count +=1
            unit_width += max_char_width + char_spacing_x
        
        # Move to the next line
        start_y += constants.MAIN_FONT.get_height() + char_spacing_y  # Add some spacing between lines
        #print(char_count)
...
def display_terrain(terrain, colour, x, y, char_spacing_x, char_spacing_y):
    start_y = y
    
    # Find the maximum width of characters
    max_char_width = max(constants.MAIN_FONT.size(char)[0] for line in terrain for char in line)
