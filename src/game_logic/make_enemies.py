...
def update_enemy_tank_positions(number_of_enemy_tanks,enemy_tank_accelerations,enemy_tank_x_positions,enemy_tank_y_positions):
    for i in range(number_of_enemy_tanks):
        if enemy_tank_accelerations[i] > 0 and enemy_tank_x_positions[i] < constants.SCREEN_WIDTH: # if tank is moving left to right and inside screen limit
            enemy_tank_x_positions[i] += enemy_tank_accelerations[i]
            if  enemy_tank_x_positions[i] + constants.ENEMY_WIDTH >= constants.SCREEN_WIDTH: #if x pos at screen max width limit
                enemy_tank_x_positions[i] = constants.SCREEN_WIDTH - constants.ENEMY_WIDTH
                enemy_tank_y_positions[i] += constants.ENEMY_HEIGHT
                enemy_tank_accelerations [i] = - enemy_tank_accelerations[i]
                display_ascii.display_unit(level_one_ascii_units.enemy_tank["straight"],constants.ENEMY_TANK_COLOUR,enemy_tank_x_positions[i],enemy_tank_y_positions[i],constants.CHAR_SPACING_X,constants.CHAR_SPACING_Y)
            else:
                display_ascii.display_unit(level_one_ascii_units.enemy_tank["straight"],constants.ENEMY_TANK_COLOUR,enemy_tank_x_positions[i],enemy_tank_y_positions[i],constants.CHAR_SPACING_X,constants.CHAR_SPACING_Y)
        elif enemy_tank_accelerations[i] < 0 and enemy_tank_y_positions[i] > constants.SCREEN_X_MIN:
            enemy_tank_x_positions[i] += enemy_tank_accelerations[i]
            if  enemy_tank_x_positions[i] <= constants.SCREEN_X_MIN: #if x pos at screen min limit
